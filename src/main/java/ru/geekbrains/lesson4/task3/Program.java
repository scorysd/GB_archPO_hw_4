...
class Database{
    ...
    public Database() {
        tickets.add(new Ticket(0001, 1, new Date(), "123", true));
        tickets.add(new Ticket(0002, 1, new Date(), "345", true));
        tickets.add(new Ticket(0003, 2, new Date(), "456", true));
        tickets.add(new Ticket(0004, 3, new Date(), "678", true));
        tickets.add(new Ticket(0005, 4, new Date(), "789", true));
...
class MobileApp{
    ...
    public MobileApp(TicketProvider ticketProvider, CustomerProvider customerProvider) {
        ...
        customer = customerProvider.getCustomer(1);
...
class CustomerProvider{
    ...
    public Customer getCustomer(int id){
        Collection<Customer> cc = database.getCustomers();
        for (Customer c: cc){
            if (c.getId() == id)
                return c;
        }
    ...
    //    Предусловие
    public CustomerProvider(Database database) throws DataBaseException {
        if (database == null){
